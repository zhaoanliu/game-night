...
Adapt before use: step 4 references a non-blocking E2E workflow (`e2e-local.yml`) — rename it to your repo's async workflow, or delete step 4 if every check is a required PR check.
...
4. **Check for any non-blocking E2E run on the PR branch before merging:**
   ```bash
   RUN=$(gh run list --workflow=e2e-local.yml --branch "$BRANCH" --limit 1 --json databaseId,status,conclusion,url --jq '.[0] // empty')
   ```
   - If `RUN` is empty → no run found for this branch; skip and proceed.
   - If `.status` is `"in_progress"` or `"queued"` → report "E2E is running, waiting for it to finish…" and:
     ```bash
     gh run watch <databaseId> --exit-status || true
     CONCLUSION=$(gh run view <databaseId> --json conclusion --jq '.conclusion')
     ```
   - If conclusion is `"failure"` → report "E2E failed on this branch (<url>). Fix the failures before merging." and **stop** — do not merge.
   - If conclusion is `"success"` → report "E2E passed ✓" and continue.
